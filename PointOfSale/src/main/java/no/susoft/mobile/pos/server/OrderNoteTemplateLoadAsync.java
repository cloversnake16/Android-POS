...
public class OrderNoteTemplateLoadAsync extends AsyncTask<String, Void, String> {
    ...
    protected String doInBackground(String... shopId) {
        ...
        try {
            ...
            request.appendOperation(OperationCode.REQUEST_ORDERNOTE_TEMPLATE);
    ...
    protected void onPostExecute(String s) {
        ...
        if (result != null) {
			...
			if (!result[0].trim().isEmpty()) {
				AppConfig.getState().setOrderNote(result[0]);
				editor.putString(Preferences.ORDER_NOTE.toString(), result[0]);
			} else {
